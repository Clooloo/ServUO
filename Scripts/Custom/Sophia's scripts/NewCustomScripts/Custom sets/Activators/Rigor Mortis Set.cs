using System;
using System.Collections;
using Server;
using Server.Prompts;
using Server.Mobiles;
using Server.Network;
using Server.Gumps;
using Server.Items;
using Server.Engines.VeteranRewards;
 
namespace Server.Items
{
    public class RigorMortis : Item
    {

        [Constructable]
        public RigorMortis() : base( 0x2AAA )
        {
            Movable = true;
            Hue = 1390;
            Weight = 25.0;
            Name = "Rigor Mortis Set";
        }
        public override void GetProperties(ObjectPropertyList list)
            {
 
            base.GetProperties(list);
            
            list.Add("The death is not the end...");
            }
       
        public override void OnDoubleClick( Mobile from )
        {
            //if (from.Race == Race.Human)
            //{  
                Item EquipArmor1 = from.FindItemOnLayer(Layer.Pants);
                Item EquipArmor2 = from.FindItemOnLayer(Layer.Gloves);
                Item EquipArmor3 = from.FindItemOnLayer(Layer.InnerTorso);
                Item EquipArmor4 = from.FindItemOnLayer(Layer.Arms);   
                //Item EquipArmor5 = from.FindItemOnLayer(Layer.Neck);
                Item EquipArmor5 = from.FindItemOnLayer(Layer.Helm);
                //Item EquipArmor7 = from.FindItemOnLayer(Layer.Cloak);//Layer.Cloak
               
 
                if (EquipArmor1 is RigorLegs || EquipArmor2 is RigorGloves || EquipArmor3 is RigorChest ||
                    EquipArmor4 is RigorArms || EquipArmor2 is RigorHelm)
                {
                	
                    //from.LocalOverheadMessage(MessageType.Emote, 0x59, true, "You remove the armor! ");
                    EquipArmor1.Delete();
                    EquipArmor2.Delete();
                    EquipArmor3.Delete();
                    EquipArmor4.Delete();
                    EquipArmor5.Delete();
                    //EquipArmor6.Delete();
                    //EquipArmor7.Delete();
                    from.Skills.Necromancy.Base -= 10;
                    from.Skills.SpiritSpeak.Base -= 10;
                    from.Skills.Magery.Base -= 10;
                    from.Skills.EvalInt.Base -= 10;
                    from.RawInt -= 30;
                    //from.RawStr -= 10;
                    return;
                }
                else if (EquipArmor1 is LegsOfBaal || EquipArmor1 is BardLegs || EquipArmor1 is WorkerLegs || EquipArmor1 is NaturalLegs)
                {
                from.LocalOverheadMessage(MessageType.Emote, 0x59, true, "Please remove the current armor set first.");
                return;
                }
                else if ( !IsChildOf( from.Backpack ) )
      {
      from.SendLocalizedMessage(1042001);
      return;
      }
                
               
                if (EquipArmor1 != null)              
                    from.AddToBackpack(EquipArmor1);
               
                if (EquipArmor2 != null)
                    from.AddToBackpack(EquipArmor2);
               
                if (EquipArmor3 != null)
                    from.AddToBackpack(EquipArmor3);
               
                if (EquipArmor4 != null)
                    from.AddToBackpack(EquipArmor4);

                 if (EquipArmor5 != null)
                    from.AddToBackpack(EquipArmor5);

                 /*if (EquipArmor6 != null)
                    from.AddToBackpack(EquipArmor6);

                if (EquipArmor7 != null)
                    from.AddToBackpack(EquipArmor7);*/
       

                //from.LocalOverheadMessage(MessageType.Emote, 0x59, true, "You feel the power of Baal on your hands! ");
                
                
                
                from.EquipItem(new RigorLegs());
                from.EquipItem(new RigorGloves());
                from.EquipItem(new RigorChest());
                from.EquipItem(new RigorArms());
                //from.EquipItem(new GorgetOfBaal());
                from.EquipItem(new RigorHelm());
                //from.EquipItem(new Baalcloak());
                //from.RawStr += 30;
                from.Skills.Necromancy.Base += 10;
                from.Skills.SpiritSpeak.Base += 10;
                from.Skills.Magery.Base += 10;
                from.Skills.EvalInt.Base += 10;
                from.RawInt += 30;
                //from.RawStr += 25;
                
                
 
            
        }
   

     
        
        public RigorMortis( Serial serial ) : base( serial )
        {
        }
 
        public override void Serialize( GenericWriter writer )
        {
            base.Serialize( writer );
 
            writer.Write( (int) 0 ); // version
        }
 
        public override void Deserialize( GenericReader reader )
        {
            base.Deserialize( reader );
 
            int version = reader.ReadInt();
        }
    }
}