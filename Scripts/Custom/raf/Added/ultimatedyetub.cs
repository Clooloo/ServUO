        ...
        if (target is BaseJewel || target is BaseArmor || target is BaseClothing || target is BaseShield || target is BaseWeapon || target is EtherealMount || target is BaseSuit || target is BaseContainer || target is Item )
            ...
            if (item.RootParent == from) // Make sure its in their pack or they are wearing it
                if (target is EtherealMount)
                    {
                      EtherealMount emitem = (EtherealMount)target;
                      emitem.TransparentMountedHue = m_Item.Hue;
                      emitem.NonTransparentMountedHue = m_Item.Hue;
                      emitem.StatueHue = m_Item.Hue;
                    }
                else item.Hue = m_Item.Hue;
            else
