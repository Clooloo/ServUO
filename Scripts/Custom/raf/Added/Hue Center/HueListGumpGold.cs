...
namespace Server.Gumps
{
	public class HueListGumpGold : Gump
	{
		...
		public HueListGumpGold(Mobile from, int page) : base(20, 30)
		{
			...
			AddHtml(0, 20, width, 50, "<basefont color=yellow><center>THE GROVE HUE CENTER", false, false);//Edit the title 

			AddLabel(width - 175, 40, 1271, String.Format("100k Per Tub"));
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			switch (info.ButtonID)
			{
				...
				default:
				{
					...
					//	Item item = state.Mobile.Backpack.FindItemByType(typeof (GoldLedger));// area defines how your to pay for the hues. currently set to gold ledger.
					//	GoldLedger ledger = item as GoldLedger;

					//	if (ledger != null && ledger.Gold >= 100000)
					//	{
					//		ledger.Gold -= 100000;
					...
					//		state.Mobile.SendGump(new HueListGumpGold(state.Mobile, pageHueFound));
					//	}
						//state.Mobile.SendMessage("There was not enough gold in your ledger to purchase a dye tub.");
