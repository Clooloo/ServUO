...
namespace Daat99MasterLooterSystem
{
    ...
    {		
		public List<Server.Mobiles.Property> Properties { get; set; }
		...
		public override void GetProperties(ObjectPropertyList list)
		{
			...
			if (owner != null && GoldLedger)
			{
				// Display the AccountGold of the player
				list.Add(1060660, "Gold\t" + owner.AccountGold.ToString());
			}
			else
			{
				// Display "Inactive" if GoldLedger is not active or owner is not found
				list.Add(1060660, "Gold\tInactive");
			}
		...
		public void Loot( PlayerMobile player )
		{
			...
			foreach ( Item item in player.GetItemsInRange(5) )
			{
				...
				{
					items.Add(item); // Correct placement of items.Add(item)
					//if (item is Gold gold)
					//{
						//gold.Delete(); // Remove the gold item from the world
					//}
				}
 			}
				...
				if ( lootContainer(player, corpse) )
				{
					...
					if ( DeleteAllCorpses || corpse.GetAmount(typeof(Item), false) == 0 )
					{
						...
						if ( reward > 0 )
						{
							...
							Gold goldReward = new Gold(reward);

							// Use TryDropItem to add the gold to the backpack, triggering AddCurrency
							this.TryDropItem(player, goldReward, false);
						}
		...
		{    
			// Assuming this class is a type of Container or Item
    		PlayerMobile player = this.RootParent as PlayerMobile;

			if ( AddCurrency(dropped, player as PlayerMobile) )
				return;
			base.DropItem(dropped);
		}
		public override bool OnDragDropInto( Mobile from, Item item, Point3D p )
		{
			if ( AddCurrency(item, from as PlayerMobile) )
				return true;
			return base.OnDragDropInto(from, item, p);
		}
		public override bool TryDropItem( Mobile from, Item dropped, bool sendFullMessage )
		{
			if ( AddCurrency(dropped, from as PlayerMobile ) )
				return true;
		...
		public void AddCurrencyFromBackpack( PlayerMobile player )
		{
			foreach ( Item item in player.Backpack.FindItemsByType(typeof(Item)) )
				AddCurrency(item, player);
		}
		public bool AddCurrency( Item currency, PlayerMobile player )
		{
			if ( !Daat99MasterLootersUtils.IsCurrencyType(currency.GetType()) )
				return false;

			if (currency is Gold && GoldLedger)
				return AddGoldPile(currency as Gold, player as PlayerMobile);

			if (currency is BankCheck && GoldLedger)
				return AddGoldCheck(currency as BankCheck, player as PlayerMobile);
		...
		public bool AddGoldPile(Gold gold, PlayerMobile player)
		{
			if (!GoldLedger || player == null)
				return false;

			AddGoldAmount((ulong)gold.Amount, player); // Deposit gold amount into the player's account.			
			base.DropItem(gold);
			gold.Delete(); // Remove the gold item from the game.
			return true;
		}

		public bool AddGoldCheck(BankCheck check, PlayerMobile player)
		{
			if (!GoldLedger || player == null)
				return false;

			AddGoldAmount((ulong)check.Worth, player); // Deposit the check's worth into the player's account
			check.Consume(); // Remove the check from the game

			return true;
		}

		public void AddGoldAmount(ulong amount, PlayerMobile player)
		{
			if (player == null || !GoldLedger)
				return; // Do nothing if the player is null or GoldLedger is not active.

			player.Account.DepositGold((int)amount); // Deposit the entire amount into the player's account.
		}
