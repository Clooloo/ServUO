...
namespace Daat99MasterLooterSystem
{
	...
	public class MasterLooterLedgerGump : Gump
    {
		...
		{
			this.backpack  = backpack;
			// Attempt to get the container of the backpack
			Item backpackContainer = backpack.Parent as Item;
			// Then, attempt to get the player from the parent of the backpack's container
			PlayerMobile player = backpackContainer.Parent as PlayerMobile;
			this.Closable=true;
			...
			if ( gold )
			{
				this.AddLabel(25, 50, 69, "You have " + player.AccountGold + " gold.");
				offset += 25;
			}
			if ( token )
			{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			else
			{
				...
				if ( amountRelay != null )
				{
					...
					if ( amount > 0 )
					{
						if ( info.ButtonID == (int)BUTTONS.GOLD && amount < 60001)
						{
							if (Banker.Withdraw(from, amount)) 
							{
							from.SendMessage(1173, "You extracted {0} gold from your bank.", amount);							
							from.AddToBackpack( new Gold((int)amount) );
							}
							else
							{
							from.SendMessage("You do not have enough funds to withdraw that amount");
							}
						}
						else if ( info.ButtonID == (int)BUTTONS.GOLD_CHECK && amount < 100000001)
						{
							if (Banker.Withdraw(from, amount)) 
							{
							from.SendMessage(1173, "You extracted {0} gold from your bank as a check.", amount);
